...
/**
 * This sample application demonstrates basic usage of the MQTT v3 Client api.
 * 
 * It can be run in one of two modes: - as a publisher, sending a single message
 * to a topic on the server - as a subscriber, listening for messages from the
 * server
 * 
 */
public class Sample implements MqttCallback {
	...
	try {
	    ...
	    // if (action.equals("publish")) {
	    sampleClient.subscribe("random", 2);
	    sampleClient.publish("random", 2, "randomMessage".getBytes());
     ...
     * @throws Exception
     */
    public void publish(String topicName, int qos, byte[] payload) throws Exception {
	...
	System.out.println("Time:\t" + time + "  Topic:\t" + topic.getName() + "  Message:\t"
